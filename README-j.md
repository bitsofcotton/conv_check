...
また、largest_intercept は使用する精度が小さいと問題に対してかなりシビアな値になり得ます。適宜調整してください。  
...
# 仕様
maximize, minimize 関数は初期値を仮定していない実装のために不安定です。
これは、一旦内点をとった後、そこからの差分として広げる形で実装しなおすと安定すると考えられますが、実装されていません。
主な目的である内点を確定する事には不要です。
